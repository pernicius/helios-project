...
namespace Helios::Engine {
	...
	void EventManager::Subscribe(EventType eventType, Scope<IEventHandlerWrapper>&& handler, EventId eventId)
	{
		if (eventId) {
			...
			if (subscribers != m_subscribers.end()) {
				...
				for (auto& it : handlers) {
					if (it->GetType() == handler->GetType()) {
						LOG_CORE_ASSERT(false, "Attempting to double-register callback");
						return;
					}
				}
	...
	void EventManager::QueueEvent(Scope<Event>&& event_, EventId eventId)
	{
